...
function Products() {
  ...
  const { data: categories } = useQuery({
    queryKey: ["categories"],
    queryFn: async () => {
      return await fetch(`${API_URL}/products/categories`).then((res) =>
        res.json()
      );
    },
    staleTime: 20000,
  ...
  const { data: products } = useQuery({
    ...
    queryFn: async () => {
      let url = `${API_URL}/products/search?limit=${limit}&skip=${skip}&q=${query}`;
      if (category) {
          ...
          <div>
            <div className="relative mt-2 rounded-md flex items-center gap-8 mb-4">
              <input
                ...
                placeholder="IPhone"
              />
              <select className="border p-2" onChange={handleSelectCategory}>
